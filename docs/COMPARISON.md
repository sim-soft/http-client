...
Measured on 2026-09-07 against the current stable release of each library:
Simsoft HttpClient 2.4.0, Guzzle 8.2.0, Symfony HttpClient 7.4.18, and Laravel
13.30.1. Package counts and sizes come from a real `composer install --no-dev`
into an empty project, so they are what a consumer actually downloads.
