...
A fluent PHP HTTP client built on `ext-curl`. PSR-7/PSR-18 compliant,
concurrent requests, built-in retry, middleware, and test doubles — all in a
single lightweight package whose only dependencies are the PSR interface
packages, which ship no implementation code.
